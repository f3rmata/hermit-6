#!/usr/bin/env bash

set -euo pipefail

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
requested_mode=${MODE-}
requested_run_id=${RUN_ID-}
requested_result_dir=${RESULT_DIR-}
# shellcheck source=common.sh
source "$SCRIPT_DIR/common.sh"

usage() {
  cat <<'USAGE'
usage:
  MODE=cgroup-hermit BINDING=redis YCSB_RECORDCOUNT=8192 \
    ./run_ycsb_page_sweep.sh

YCSB page-size sweep under Hermit RDMA swap. For every requested THP page size
the script starts a KV server (Redis or Memcached) in a cgroup, loads a YCSB
dataset, lowers memory.max to force RDMA stores, restores memory.max, then runs
a read-only YCSB workload (default workloadc) and snapshots Hermit order_stats
around the run to compute remote-load bytes, large-folio share, protocol
bandwidth, throughput and latency percentiles.

Important environment:
  BINDING=redis                     (redis or memcached)
  YCSB_BIN=$HOME/ycsb-0.17.0/bin/ycsb
  PAGE_SIZES_KB="4 16 32 64 128 256 512 1024 2048"
  YCSB_WORKSET_MB=16384             (target total value bytes; used only for
                                     sanity checks, recordcount controls size)
  YCSB_RECORDCOUNT=8192             (keys to load; default fits 2 MiB * 16 GiB)
  YCSB_OPERATIONCOUNT=8192          (run-phase ops; default = recordcount)
  YCSB_FIELDLENGTH=2097152          (bytes per field; default 2 MiB)
  YCSB_FIELDCOUNT=1                 (fields per record; default 1)
  YCSB_READALLFIELDS=true           (whole-record GETs)
  YCSB_REQUESTDISTRIBUTION=uniform  (uniform | zipfian)
  YCSB_WORKLOAD=custom              (custom generated CoreWorkload)
  LOCAL_RATIO_PCT=70                (cgroup memory.max = resident * ratio / 100)
  BENCH_REPEATS=3
  BENCH_CPUS=0                      (YCSB Java client CPU set)
  SERVER_CPUS=0-7                   (Redis/Memcached server CPU set)
  REDIS_PORT=6391
  MEMCACHED_PORT=11211
  MEMCACHED_MAX_MB=32768            (memcached -m; large enough to avoid eviction)
  MEMCACHED_MAX_ITEM_MB=4           (memcached -I; must exceed serialized value)
  QUIET_INTERVAL_SEC=1
  QUIET_SAMPLES=3
  QUIET_TIMEOUT_SEC=120
  READY_TIMEOUT_SEC=600
  BENCH_TIMEOUT_SEC=3600
  RESTORE_THP=1

Requirements: cgroup v2, active Hermit RDMA backend, debugfs, zswap disabled,
exactly one block-device swap area, and a YCSB 0.17.0 distribution with the
selected binding installed. The script owns /sys/fs/cgroup/hermit-ycsb.
USAGE
}

if [ "${1:-}" = -h ] || [ "${1:-}" = --help ]; then
  usage
  exit 0
fi

MODE=$(normalize_mode "${requested_mode:-cgroup-hermit}")
[ "$MODE" = cgroup-hermit ] || rdma_die "this benchmark requires MODE=cgroup-hermit"

BINDING=${BINDING:-redis}
case "$BINDING" in
  redis|memcached) ;;
  *) rdma_die "BINDING must be redis or memcached" ;;
esac

PAGE_SIZES_KB=${PAGE_SIZES_KB:-"4 16 32 64 128 256 512 1024 2048"}
YCSB_BIN=${YCSB_BIN:-"$HOME/ycsb-0.17.0/bin/ycsb"}
YCSB_RECORDCOUNT=${YCSB_RECORDCOUNT:-8192}
YCSB_OPERATIONCOUNT=${YCSB_OPERATIONCOUNT:-$YCSB_RECORDCOUNT}
YCSB_FIELDLENGTH=${YCSB_FIELDLENGTH:-2097152}
YCSB_FIELDCOUNT=${YCSB_FIELDCOUNT:-1}
YCSB_READALLFIELDS=${YCSB_READALLFIELDS:-true}
YCSB_REQUESTDISTRIBUTION=${YCSB_REQUESTDISTRIBUTION:-uniform}
LOCAL_RATIO_PCT=${LOCAL_RATIO_PCT:-70}
BENCH_REPEATS=${BENCH_REPEATS:-3}
BENCH_CPUS=${BENCH_CPUS:-0}
SERVER_CPUS=${SERVER_CPUS:-0-7}
REDIS_PORT=${REDIS_PORT:-6391}
MEMCACHED_PORT=${MEMCACHED_PORT:-11211}
MEMCACHED_MAX_MB=${MEMCACHED_MAX_MB:-32768}
MEMCACHED_MAX_ITEM_MB=${MEMCACHED_MAX_ITEM_MB:-4}
QUIET_INTERVAL_SEC=${QUIET_INTERVAL_SEC:-1}
QUIET_SAMPLES=${QUIET_SAMPLES:-3}
QUIET_TIMEOUT_SEC=${QUIET_TIMEOUT_SEC:-120}
READY_TIMEOUT_SEC=${READY_TIMEOUT_SEC:-600}
BENCH_TIMEOUT_SEC=${BENCH_TIMEOUT_SEC:-3600}
RESTORE_THP=${RESTORE_THP:-1}
RUN_ID=${requested_run_id:-"$(date +%Y%m%d-%H%M%S)-ycsb-swapio"}
RESULT_DIR=${requested_result_dir:-"$RESULT_ROOT/$RUN_ID"}
CGROUP=/sys/fs/cgroup/hermit-ycsb
THP_ROOT=/sys/kernel/mm/transparent_hugepage
REMOTE_MASK_FILE=/sys/kernel/debug/hermit/remote_order_mask
EFFECTIVE_MASK_FILE=/sys/kernel/debug/hermit/effective_order_mask
CSV="$RESULT_DIR/ycsb-swapio-summary.csv"
current_server_pid=
current_server_log=
remote_mask_before=
declare -a thp_files=()
declare -A thp_policy=()

page_order() {
  case "$1" in
    4) printf 0 ;; 16) printf 2 ;; 32) printf 3 ;; 64) printf 4 ;;
    128) printf 5 ;; 256) printf 6 ;; 512) printf 7 ;;
    1024) printf 8 ;; 2048) printf 9 ;; *) return 1 ;;
  esac
}

selected_policy() {
  awk '{ for (i = 1; i <= NF; i++) if ($i ~ /^\[.*\]$/) { gsub(/\[|\]/, "", $i); print $i; exit } }' "$1"
}

find_redis_server_bin() {
  local bin
  for bin in "${REDIS_SERVER_BIN:-}" "$PWD/redis/src/redis-server" "$HOME/redis/src/redis-server"; do
    if [ -n "$bin" ] && [ -x "$bin" ]; then
      printf '%s' "$bin"
      return 0
    fi
  done
  if command -v redis-server >/dev/null 2>&1; then
    command -v redis-server
    return 0
  fi
  rdma_die "cannot find redis-server; set REDIS_SERVER_BIN=/path/to/redis-server"
}

find_memcached_server_bin() {
  local bin
  for bin in "${MEMCACHED_BIN:-}" "$PWD/memcached/memcached" "$HOME/memcached/memcached"; do
    if [ -n "$bin" ] && [ -x "$bin" ]; then
      printf '%s' "$bin"
      return 0
    fi
  done
  if command -v memcached >/dev/null 2>&1; then
    command -v memcached
    return 0
  fi
  rdma_die "cannot find memcached; set MEMCACHED_BIN=/path/to/memcached"
}

snapshot_order_stats() {
  sudo_cat /sys/kernel/debug/hermit/order_stats
}

order_field() {
  local file=$1 order=$2 column=$3
  awk -v o="$order" -v c="$column" 'NR > 1 && $1 == o { print $c; found=1 } END { if (!found) print 0 }' "$file"
}

order_total_stores() {
  snapshot_order_stats | awk 'NR > 1 { total += $3 } END { printf "%d", total }'
}

wait_for_stores_quiet() {
  local previous current quiet=0 start
  start=$(date +%s)
  previous=$(order_total_stores)
  while [ "$quiet" -lt "$QUIET_SAMPLES" ]; do
    sleep "$QUIET_INTERVAL_SEC"
    current=$(order_total_stores)
    if [ "$current" -eq "$previous" ]; then
      quiet=$((quiet + 1))
    else
      previous=$current
      quiet=0
    fi
    if [ "$quiet" -lt "$QUIET_SAMPLES" ]; then
      [ "$(( $(date +%s) - start ))" -lt "$QUIET_TIMEOUT_SEC" ] || \
        rdma_die "Hermit store counters did not become quiet; stop other swap workloads"
    fi
  done
}

memory_event() {
  local key=$1
  awk -v key="$key" '$1 == key { print $2; found=1 } END { if (!found) print 0 }' \
    "$CGROUP/memory.events"
}

save_thp_state() {
  local file policy
  for file in "$THP_ROOT/enabled" "$THP_ROOT"/hugepages-*kB/enabled; do
    [ -e "$file" ] || continue
    policy=$(selected_policy "$file")
    [ -n "$policy" ] || rdma_die "cannot read THP policy: $file"
    thp_files+=("$file")
    thp_policy["$file"]=$policy
  done
  remote_mask_before=$(sudo_cat "$REMOTE_MASK_FILE")
}

configure_page_size() {
  local kb=$1 order=$2 file selected
  for file in "${thp_files[@]}"; do
    sudo_write never "$file"
  done
  if [ "$kb" != 4 ]; then
    selected="$THP_ROOT/hugepages-${kb}kB/enabled"
    if [ "$kb" = 2048 ] && [ ! -e "$selected" ]; then
      selected="$THP_ROOT/enabled"
    fi
    [ -e "$selected" ] || return 1
    sudo_write always "$selected"
    [ "$kb" != 2048 ] || sudo_write always "$THP_ROOT/enabled"
  fi
  sudo_write "$(printf '0x%x' $((1 | (1 << order))))" "$REMOTE_MASK_FILE"
}

start_server() {
  local run_dir=$1 bin
  current_server_log="$run_dir/server.log"
  if [ "$BINDING" = redis ]; then
    bin=$(find_redis_server_bin)
    taskset -c "$SERVER_CPUS" "$bin" \
      --port "$REDIS_PORT" --bind 127.0.0.1 --protected-mode no \
      --save "" --appendonly no --daemonize no \
      --stop-writes-on-bgsave-error no --rdbcompression no --maxmemory 0 \
      > "$current_server_log" 2>&1 &
    current_server_pid=$!
  else
    bin=$(find_memcached_server_bin)
    taskset -c "$SERVER_CPUS" "$bin" \
      -l 127.0.0.1 -p "$MEMCACHED_PORT" -U 0 \
      -m "$MEMCACHED_MAX_MB" -c 32768 \
      -I "${MEMCACHED_MAX_ITEM_MB}m" -t 8 \
      > "$current_server_log" 2>&1 &
    current_server_pid=$!
  fi
  sudo_write "$current_server_pid" "$CGROUP/cgroup.procs"
}

assert_server_alive() {
  local context=$1
  kill -0 "$current_server_pid" 2>/dev/null || \
    rdma_die "server exited $context; see $current_server_log"
}

wait_for_server_ready() {
  local start pattern
  start=$(date +%s)
  if [ "$BINDING" = redis ]; then
    pattern='Ready to accept connections'
  else
    pattern='server listening'
  fi
  until grep -q "$pattern" "$current_server_log" 2>/dev/null; do
    assert_server_alive "during startup"
    [ "$(( $(date +%s) - start ))" -lt "$READY_TIMEOUT_SEC" ] || \
      rdma_die "timeout waiting for server; see $current_server_log"
    sleep 0.2
  done
}

cleanup_processes() {
  if [ -n "$current_server_pid" ] && kill -0 "$current_server_pid" 2>/dev/null; then
    kill -TERM "$current_server_pid" 2>/dev/null || true
    wait "$current_server_pid" 2>/dev/null || true
  fi
  current_server_pid=
  current_server_log=
  sudo_write max "$CGROUP/memory.max" 2>/dev/null || true
}

cleanup() {
  local ret=$? file
  cleanup_processes
  if [ "$RESTORE_THP" = 1 ]; then
    for file in "${thp_files[@]}"; do
      sudo_write "${thp_policy[$file]}" "$file" || true
    done
    [ -z "$remote_mask_before" ] || sudo_write "$remote_mask_before" "$REMOTE_MASK_FILE" || true
  fi
  sudo rmdir "$CGROUP" 2>/dev/null || true
  exit "$ret"
}

ycsb_parse_field() {
  local file=$1 section=$2 field=$3
  awk -F', ' -v sec="$section" -v key="$field" '
    $1 == "[" sec "]" {
      name = $2
      gsub(/^ +| +$/, "", name)
      if (name == key) { value = $3 }
    }
    END {
      if (value == "") value = 0
      print value
    }
  ' "$file"
}

generate_workload_file() {
  local file=$1 port=$2
  cat > "$file" <<EOF
workload=site.ycsb.workloads.CoreWorkload
recordcount=$YCSB_RECORDCOUNT
operationcount=$YCSB_OPERATIONCOUNT
fieldcount=$YCSB_FIELDCOUNT
fieldlength=$YCSB_FIELDLENGTH
readallfields=$YCSB_READALLFIELDS
readproportion=1.0
updateproportion=0.0
scanproportion=0.0
insertproportion=0.0
requestdistribution=$YCSB_REQUESTDISTRIBUTION
EOF
  if [ "$BINDING" = redis ]; then
    cat >> "$file" <<EOF
redis.host=127.0.0.1
redis.port=$port
EOF
  else
    cat >> "$file" <<EOF
memcached.hosts=127.0.0.1:$port
EOF
  fi
}

run_ycsb_phase() {
  local phase=$1 workload_file=$2 log=$3
  timeout "$BENCH_TIMEOUT_SEC" taskset -c "$BENCH_CPUS" \
    "$YCSB_BIN" "$phase" "$BINDING" -P "$workload_file" > "$log" 2>&1
}

# ---------------------------------------------------------------------------
# Validation
# ---------------------------------------------------------------------------

[ -x "$YCSB_BIN" ] || rdma_die "missing YCSB launcher: $YCSB_BIN"
[[ "$YCSB_RECORDCOUNT" =~ ^[0-9]+$ ]] && [ "$YCSB_RECORDCOUNT" -gt 0 ] || \
  rdma_die "YCSB_RECORDCOUNT must be a positive integer"
[[ "$YCSB_OPERATIONCOUNT" =~ ^[0-9]+$ ]] && [ "$YCSB_OPERATIONCOUNT" -gt 0 ] || \
  rdma_die "YCSB_OPERATIONCOUNT must be a positive integer"
[[ "$YCSB_FIELDLENGTH" =~ ^[0-9]+$ ]] && [ "$YCSB_FIELDLENGTH" -gt 0 ] || \
  rdma_die "YCSB_FIELDLENGTH must be a positive integer"
[[ "$YCSB_FIELDCOUNT" =~ ^[0-9]+$ ]] && [ "$YCSB_FIELDCOUNT" -gt 0 ] || \
  rdma_die "YCSB_FIELDCOUNT must be a positive integer"
case "$YCSB_REQUESTDISTRIBUTION" in
  uniform|zipfian|sequential|latest) ;;
  *) rdma_die "YCSB_REQUESTDISTRIBUTION must be uniform, zipfian, sequential or latest" ;;
esac
YCSB_VALUE_SIZE=$((YCSB_FIELDLENGTH * YCSB_FIELDCOUNT))
if [ "$YCSB_VALUE_SIZE" -lt 65536 ]; then
  rdma_log "warning: YCSB value size is only $YCSB_VALUE_SIZE bytes; large-page benefit will be weak"
fi
if [ "$BINDING" = memcached ] && [ "$YCSB_VALUE_SIZE" -gt $((MEMCACHED_MAX_ITEM_MB * 1024 * 1024)) ]; then
  rdma_die "YCSB_VALUE_SIZE exceeds memcached -I ${MEMCACHED_MAX_ITEM_MB}m; raise MEMCACHED_MAX_ITEM_MB"
fi
taskset -c "$BENCH_CPUS" true >/dev/null 2>&1 || \
  rdma_die "invalid or unavailable BENCH_CPUS CPU list: $BENCH_CPUS"
taskset -c "$SERVER_CPUS" true >/dev/null 2>&1 || \
  rdma_die "invalid or unavailable SERVER_CPUS CPU list: $SERVER_CPUS"
[ -r /sys/fs/cgroup/cgroup.controllers ] || rdma_die "cgroup v2 is required"
mount_debugfs_if_needed
sudo_test -r /sys/kernel/debug/hermit/order_stats || rdma_die "Hermit order_stats is unavailable"
sudo_test -w "$REMOTE_MASK_FILE" || rdma_die "Hermit remote_order_mask is not writable"
[ "$(detect_rswap_backend)" = rdma ] || rdma_die "Hermit RDMA backend is not active"
swap_count=$(awk 'NR > 1 { count++ } END { print count + 0 }' /proc/swaps)
[ "$swap_count" -eq 1 ] || \
  rdma_die "exactly one active swap device is required; found $swap_count"
awk 'NR == 2 && $2 == "partition" { ok=1 } END { exit !ok }' /proc/swaps || \
  rdma_die "the active swap area must be a block device, not a swapfile"
if [ -r /sys/module/zswap/parameters/enabled ] && grep -qi '^Y' /sys/module/zswap/parameters/enabled; then
  rdma_die "zswap is enabled; disable it before measuring RDMA swap-out"
fi

trap cleanup EXIT
trap 'exit 130' INT
trap 'exit 143' TERM
save_thp_state
configure_hermit_mode

if sudo_test -d "$CGROUP"; then
  existing=$(sudo_cat "$CGROUP/cgroup.procs")
  [ -z "$existing" ] || rdma_die "$CGROUP is busy with PIDs: $existing"
else
  sudo mkdir "$CGROUP"
fi
sudo_write max "$CGROUP/memory.max"
sudo_write max "$CGROUP/memory.swap.max"

mkdir -p "$RESULT_DIR"
{
  printf 'date=%s\n' "$(date --iso-8601=seconds)"
  printf 'kernel=%s\n' "$(uname -r)"
  printf 'mode=%s\nbinding=%s\n' "$MODE" "$BINDING"
  printf 'page_sizes_kb=%s\nrepeats=%s\n' "$PAGE_SIZES_KB" "$BENCH_REPEATS"
  printf 'bench_cpus=%s\nserver_cpus=%s\n' "$BENCH_CPUS" "$SERVER_CPUS"
  printf 'ycsb_bin=%s\n' "$YCSB_BIN"
  printf 'ycsb_recordcount=%s\nycsb_operationcount=%s\n' "$YCSB_RECORDCOUNT" "$YCSB_OPERATIONCOUNT"
  printf 'ycsb_fieldcount=%s\nycsb_fieldlength=%s\nycsb_value_size=%s\n' \
    "$YCSB_FIELDCOUNT" "$YCSB_FIELDLENGTH" "$YCSB_VALUE_SIZE"
  printf 'ycsb_readallfields=%s\nycsb_requestdistribution=%s\n' \
    "$YCSB_READALLFIELDS" "$YCSB_REQUESTDISTRIBUTION"
  printf 'local_ratio_pct=%s\n' "$LOCAL_RATIO_PCT"
  if [ "$BINDING" = redis ]; then
    printf 'redis_port=%s\nredis_server_bin=%s\n' "$REDIS_PORT" "$(find_redis_server_bin)"
  else
    printf 'memcached_port=%s\nmemcached_max_mb=%s\nmemcached_max_item_mb=%s\nmemcached_bin=%s\n' \
      "$MEMCACHED_PORT" "$MEMCACHED_MAX_MB" "$MEMCACHED_MAX_ITEM_MB" "$(find_memcached_server_bin)"
  fi
  printf 'backend=%s\n' "$(detect_rswap_backend)"
  printf 'swap:\n'
  sed 's/^/  /' /proc/swaps
} > "$RESULT_DIR/environment.txt"
printf 'page_kb,order,repeat,binding,mode,recordcount,operationcount,fieldcount,fieldlength,value_size,requestdistribution,resident_mb,limit_mb,load_sec,run_sec,throughput_ops,read_ops,actual_access_pct,useful_bytes,useful_gib_per_sec,read_avg_us,read_p95_us,read_p99_us,read_ok,pswpout_delta,target_stores_delta,target_loads_delta,target_fallback_delta,target_errors_delta,total_store_bytes,large_store_bytes,large_store_pct,swapout_gib,protocol_gib,protocol_gib_per_sec,pswpin_delta,bench_pswpout_delta,backend_loads_delta,total_load_bytes,large_load_bytes,large_load_pct,swapin_gib,load_protocol_gib,load_protocol_gib_per_sec,remote_bytes_per_useful_byte,backend_loads_per_op\n' > "$CSV"

if [ "$BINDING" = redis ]; then
  SERVER_PORT=$REDIS_PORT
else
  SERVER_PORT=$MEMCACHED_PORT
fi

for kb in $PAGE_SIZES_KB; do
  order=$(page_order "$kb") || rdma_die "unsupported page size: ${kb} KiB"
  configure_page_size "$kb" "$order" || rdma_die "THP size ${kb} KiB is unavailable"
  effective=$(sudo_cat "$EFFECTIVE_MASK_FILE")
  (( (effective & (1 << order)) != 0 )) || rdma_die "effective mask $effective lacks order $order"

  for repeat in $(seq 1 "$BENCH_REPEATS"); do
    run_dir="$RESULT_DIR/${kb}k/r${repeat}"
    mkdir -p "$run_dir"
    sudo_write max "$CGROUP/memory.max"

    start_server "$run_dir"
    wait_for_server_ready

    workload_file="$run_dir/workload.properties"
    generate_workload_file "$workload_file" "$SERVER_PORT"
    load_log="$run_dir/ycsb-load.log"
    run_log="$run_dir/ycsb-run.log"
    before="$run_dir/order-before.txt"
    after="$run_dir/order-after.txt"
    load_before="$run_dir/order-before-swapin.txt"
    load_after="$run_dir/order-after-swapin.txt"

    # ----------------------------------------------------------------------
    # Load phase
    # ----------------------------------------------------------------------
    load_start=$(date +%s%N)
    run_ycsb_phase load "$workload_file" "$load_log"
    load_end=$(date +%s%N)
    assert_server_alive "during YCSB load"
    load_sec=$(awk -v s="$load_start" -v e="$load_end" 'BEGIN { printf "%.3f", (e - s) / 1000000000 }')

    resident_bytes=$(sudo_cat "$CGROUP/memory.current")
    resident_mb=$((resident_bytes / 1024 / 1024))
    limit_mb=$(max_int 256 $((resident_mb * LOCAL_RATIO_PCT / 100)))
    [ "$limit_mb" -lt "$resident_mb" ] || \
      rdma_die "computed limit ${limit_mb} MiB is not below resident ${resident_mb} MiB; lower LOCAL_RATIO_PCT or enlarge the dataset"
    required_swap_mb=$((resident_mb - limit_mb))
    free_swap_mb=$(awk 'NR > 1 { free += $3 - $4 } END { printf "%d", free / 1024 }' /proc/swaps)
    [ "$free_swap_mb" -ge "$required_swap_mb" ] || \
      rdma_die "only ${free_swap_mb} MiB swap is free; at least ${required_swap_mb} MiB is required"

    # ----------------------------------------------------------------------
    # Swap-out phase: lower memory.max, wait for Hermit stores to go quiet
    # ----------------------------------------------------------------------
    sudo_cat "$CGROUP/memory.stat" > "$run_dir/memory-stat-before.txt"
    wait_for_stores_quiet
    snapshot_order_stats > "$before"
    pswpout_before=$(read_vmstat_key pswpout)
    stores_before=$(order_total_stores)
    oom_before=$(memory_event oom_kill)
    start_ns=$(date +%s%N)
    sudo_write "$((limit_mb * 1024 * 1024))" "$CGROUP/memory.max"
    last_change_ns=$start_ns
    previous=$(order_total_stores)
    quiet=0
    quiet_start=$(date +%s)
    while [ "$quiet" -lt "$QUIET_SAMPLES" ]; do
      sleep "$QUIET_INTERVAL_SEC"
      current=$(order_total_stores)
      if [ "$current" -ne "$previous" ]; then
        previous=$current
        quiet=0
        last_change_ns=$(date +%s%N)
      else
        quiet=$((quiet + 1))
      fi
      if [ "$quiet" -lt "$QUIET_SAMPLES" ]; then
        [ "$(( $(date +%s) - quiet_start ))" -lt "$QUIET_TIMEOUT_SEC" ] || \
          rdma_die "swap-out did not become quiet within ${QUIET_TIMEOUT_SEC}s; see $run_dir"
      fi
    done
    snapshot_order_stats > "$after"
    pswpout_after=$(read_vmstat_key pswpout)
    sudo_cat "$CGROUP/memory.stat" > "$run_dir/memory-stat-after.txt"
    sudo_cat "$CGROUP/memory.events" > "$run_dir/memory-events-after.txt"
    oom_after=$(memory_event oom_kill)
    [ "$oom_after" -eq "$oom_before" ] || rdma_die "cgroup OOM killed the server; see $run_dir"
    assert_server_alive "during reclaim"

    target_stores=$(( $(order_field "$after" "$order" 3) - $(order_field "$before" "$order" 3) ))
    target_loads=$(( $(order_field "$after" "$order" 4) - $(order_field "$before" "$order" 4) ))
    target_fallback=$(( $(order_field "$after" "$order" 5) - $(order_field "$before" "$order" 5) ))
    target_errors=$(( $(order_field "$after" "$order" 6) - $(order_field "$before" "$order" 6) ))
    pswpout_delta=$((pswpout_after - pswpout_before))
    stores_after=$(order_total_stores)
    stores_delta=$((stores_after - stores_before))

    completion_ms=$(( (last_change_ns - start_ns) / 1000000 ))
    [ "$completion_ms" -gt 0 ] || completion_ms=1
    metrics=$(awk -v before="$before" -v after="$after" -v ms="$completion_ms" '
      FILENAME == before && NR > 1 { b[$1]=$3; fb[$1]=$5; size[$1]=$2; next }
      FILENAME == after && FNR > 1 {
        d=$3-b[$1]; f=$5-fb[$1];
        if (f < 0) f=0; if (f > d) f=d;
        total += d*$2; if ($1 > 0) large += (d-f)*$2
      }
      END {
        pct=total ? 100*large/total : 0;
        gib=total/1073741824;
        ms=(ms > 0 ? ms : 1);
        printf "%.0f,%.0f,%.3f,%.6f", total, large, pct, gib/(ms/1000)
      }' "$before" "$after")
    total_store_bytes=${metrics%%,*}
    rest=${metrics#*,}; large_store_bytes=${rest%%,*}
    rest=${rest#*,}; large_store_pct=${rest%%,*}
    protocol_gib_per_sec=${rest#*,}
    swapout_gib=$(awk -v p="$pswpout_delta" 'BEGIN { printf "%.6f", p*4096/1073741824 }')
    protocol_gib=$(awk -v b="$total_store_bytes" 'BEGIN { printf "%.6f", b/1073741824 }')

    # ----------------------------------------------------------------------
    # Swap-in phase: restore memory.max, run YCSB read-only workload
    # ----------------------------------------------------------------------
    sudo_write max "$CGROUP/memory.max"
    snapshot_order_stats > "$load_before"
    pswpin_before=$(read_vmstat_key pswpin)
    bench_pswpout_before=$(read_vmstat_key pswpout)

    run_ycsb_phase run "$workload_file" "$run_log"
    assert_server_alive "during YCSB run"

    snapshot_order_stats > "$load_after"
    pswpin_after=$(read_vmstat_key pswpin)
    bench_pswpout_after=$(read_vmstat_key pswpout)
    sudo_cat "$CGROUP/memory.stat" > "$run_dir/memory-stat-after-swapin.txt"
    sudo_cat "$CGROUP/memory.events" > "$run_dir/memory-events-after-swapin.txt"

    run_time_ms=$(ycsb_parse_field "$run_log" OVERALL "RunTime(ms)")
    throughput_ops=$(ycsb_parse_field "$run_log" OVERALL "Throughput(ops/sec)")
    read_ops=$(ycsb_parse_field "$run_log" READ "Operations")
    read_avg_us=$(ycsb_parse_field "$run_log" READ "AverageLatency(us)")
    read_p95_us=$(ycsb_parse_field "$run_log" READ "95thPercentileLatency(us)")
    read_p99_us=$(ycsb_parse_field "$run_log" READ "99thPercentileLatency(us)")
    read_ok=$(ycsb_parse_field "$run_log" READ "Return=OK")
    [ "${read_ops:-0}" -gt 0 ] || rdma_die "YCSB run reported no READ operations; see $run_log"
    [ "${read_ok:-0}" -eq "$read_ops" ] || \
      rdma_die "YCSB READ failures: ok=$read_ok ops=$read_ops; see $run_log"

    run_sec=$(awk -v ms="$run_time_ms" 'BEGIN { printf "%.6f", ms / 1000 }')
    useful_bytes=$((read_ops * YCSB_VALUE_SIZE))
    useful_gib=$(awk -v b="$useful_bytes" 'BEGIN { printf "%.6f", b / 1073741824 }')
    useful_gib_per_sec=$(awk -v g="$useful_gib" -v s="$run_sec" \
      'BEGIN { printf "%.6f", (s > 0 ? g / s : 0) }')
    actual_access_pct=$(awk -v ops="$read_ops" -v rec="$YCSB_RECORDCOUNT" \
      'BEGIN { printf "%.3f", 100 * ops / rec }')

    pswpin_delta=$((pswpin_after - pswpin_before))
    bench_pswpout_delta=$((bench_pswpout_after - bench_pswpout_before))
    target_loads=$(( $(order_field "$load_after" "$order" 4) - $(order_field "$load_before" "$order" 4) ))
    target_fallback_swapin=$(( $(order_field "$load_after" "$order" 5) - $(order_field "$load_before" "$order" 5) ))
    target_errors_swapin=$(( $(order_field "$load_after" "$order" 6) - $(order_field "$load_before" "$order" 6) ))

    load_metrics=$(awk -v before="$load_before" -v after="$load_after" -v sec="$run_sec" '
      FILENAME == before && NR > 1 { b[$1]=$4; fb[$1]=$5; size[$1]=$2; next }
      FILENAME == after && FNR > 1 {
        d=$4-b[$1]; f=$5-fb[$1];
        if (f < 0) f=0; if (f > d) f=d;
        requests += d; total += d*$2; if ($1 > 0) large += (d-f)*$2
      }
      END {
        pct=total ? 100*large/total : 0;
        printf "%.0f,%.0f,%.3f,%.6f,%.0f", total, large, pct,
               (sec > 0 ? total / 1073741824 / sec : 0), requests
      }' "$load_before" "$load_after")
    [ -n "$load_metrics" ] || rdma_die "empty Hermit swap-in counter summary; see $run_dir"
    total_load_bytes=${load_metrics%%,*}
    rest=${load_metrics#*,}; large_load_bytes=${rest%%,*}
    rest=${rest#*,}; large_load_pct=${rest%%,*}
    rest=${rest#*,}; load_protocol_gib_per_sec=${rest%%,*}
    backend_loads_delta=${rest#*,}
    swapin_gib=$(awk -v p="$pswpin_delta" 'BEGIN { printf "%.6f", p*4096/1073741824 }')
    load_protocol_gib=$(awk -v b="$total_load_bytes" 'BEGIN { printf "%.6f", b/1073741824 }')
    remote_bytes_per_useful_byte=$(awk -v remote="$total_load_bytes" \
      -v useful="$useful_bytes" \
      'BEGIN { printf "%.6f", (useful > 0 ? remote / useful : 0) }')
    backend_loads_per_op=$(awk -v loads="$backend_loads_delta" -v ops="$read_ops" \
      'BEGIN { printf "%.6f", (ops > 0 ? loads / ops : 0) }')

    oom_after_swapin=$(memory_event oom_kill)
    [ "$oom_after_swapin" -eq "$oom_before" ] || rdma_die "cgroup OOM during YCSB run; see $run_dir"
    [ "$pswpin_delta" -gt 0 ] || rdma_die "YCSB run caused no swap-ins; see $run_dir"
    [[ "$total_load_bytes" =~ ^[0-9]+$ ]] && [ "$total_load_bytes" -gt 0 ] || \
      rdma_die "Hermit recorded no backend loads; see $run_dir"
    [ "$bench_pswpout_delta" -eq 0 ] || \
      rdma_die "YCSB run caused $bench_pswpout_delta new swap-outs; results are not isolated"

    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
      "$kb" "$order" "$repeat" "$BINDING" "$MODE" \
      "$YCSB_RECORDCOUNT" "$YCSB_OPERATIONCOUNT" "$YCSB_FIELDCOUNT" \
      "$YCSB_FIELDLENGTH" "$YCSB_VALUE_SIZE" "$YCSB_REQUESTDISTRIBUTION" \
      "$resident_mb" "$limit_mb" "$load_sec" "$run_sec" "$throughput_ops" \
      "$read_ops" "$actual_access_pct" "$useful_bytes" "$useful_gib_per_sec" \
      "$read_avg_us" "$read_p95_us" "$read_p99_us" "$read_ok" \
      "$pswpout_delta" "$target_stores" "$target_loads" "$target_fallback" \
      "$target_errors" "$total_store_bytes" "$large_store_bytes" \
      "$large_store_pct" "$swapout_gib" "$protocol_gib" \
      "$protocol_gib_per_sec" "$pswpin_delta" "$bench_pswpout_delta" \
      "$backend_loads_delta" "$total_load_bytes" "$large_load_bytes" \
      "$large_load_pct" "$swapin_gib" "$load_protocol_gib" \
      "$load_protocol_gib_per_sec" "$remote_bytes_per_useful_byte" \
      "$backend_loads_per_op" >> "$CSV"
    rdma_log "page=${kb}k repeat=$repeat binding=$BINDING stores=$stores_delta store_large=${large_store_pct}% swapout=${protocol_gib_per_sec}GiB/s loads=$backend_loads_delta load_large=${large_load_pct}% useful=${useful_gib_per_sec}GiB/s read_amp=${remote_bytes_per_useful_byte} ops=${throughput_ops} p99=${read_p99_us}us"
    cleanup_processes
  done
done

trap - EXIT INT TERM
cleanup
